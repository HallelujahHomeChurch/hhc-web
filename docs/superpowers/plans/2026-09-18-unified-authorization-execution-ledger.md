...
| --- | --- | --- | --- |
| R: Website hosted Account transport | `reopened / shared fix published / consumers incomplete` | The 2026-09-18 `v1.0.8` cold-session evidence remains useful historical evidence but is superseded by the 2026-09-19 real-user reproduction: after complete sign-out and Website login, first Account-profile entry flashed login UI and first Admin entry could show “OAuth 回呼失敗”; the second warm attempt succeeded. The root race was traced to post-exchange revalidation coalescing an older anonymous in-flight request. `frontend-platform` PR #65 published `1.0.9`; paused isolated Account, Admin, and Website consumer work is not yet a completed release. | Follow Phase 0 of the 2026-09-19 focused plan. Release each consumer independently, then pass the exact first-visit Website -> Account -> Admin matrix. A warm repeat is not acceptance. Preserve protected bulletins. |
...
| 6: shared AuthN runtime and consumers | `browser first-entry regression reopened / Presenter Desktop evidence pending` | The `v1.0.8` browser releases and Presenter `v2.5.9` remain released, but the later exact first-entry reproduction invalidates the browser acceptance claim. `frontend-platform` `1.0.9` contains the shared root correction; Account/Admin/Website consumer completion and cold live proof are pending. | Complete focused-plan Phase 0 before later Admin/member work, then complete Presenter Desktop physical-device/deep-link checks, Sentry redaction, and final token-rate comparison. |
...
1. Approve and complete the 2026-09-19 focused plan Phase 0 cold first-entry
   recovery before any membership, organization, or Admin-layout implementation.
