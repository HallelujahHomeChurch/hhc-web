...
| --- | --- | --- | --- |
| R: Website hosted Account transport | `Gateway live verified / Website CSP release pending` | `hhc-web` PR [#105](https://github.com/HallelujahHomeChurch/hhc-web/pull/105), merge `877a212`, changed the shared session client from same-origin `/api/account/v1` to `account.alive.org.tw`; live Website CORS preflight from `www` initially had no allow-origin header. Gateway PR [#102](https://github.com/HallelujahHomeChurch/api-gateway/pull/102), merge `672a9fe`, release [35309023148](https://github.com/HallelujahHomeChurch/api-gateway/actions/runs/35309023148), and revision `api-gateway--0000142` are healthy; live `www` preflight allows only the exact origin and an untrusted origin has no allow-origin header. Browser verification then identified a Website `connect-src` policy violation for the Account authority. Website PR [#110](https://github.com/HallelujahHomeChurch/hhc-web/pull/110), merge `998cd04`, adds the two exact production/test Account authorities; its production release [35309929662](https://github.com/HallelujahHomeChurch/hhc-web/actions/runs/35309929662) is in progress. | Website release, then real Website session/access-token evidence. |
