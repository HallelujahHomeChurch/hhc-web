...
export function HistoryTimeline({content, timeline}: HistoryTimelineProps) {
  return (
    <article className="shell mt-7 overflow-hidden rounded-2xl border border-line/80 bg-paper/90 px-[38px] py-[34px] shadow-warm max-[620px]:px-5 max-[620px]:py-6">
      <div className="mb-[34px] grid grid-cols-[minmax(0,1.15fr)_minmax(280px,.85fr)] items-center gap-[30px] overflow-hidden rounded-[14px] border border-panel-border bg-panel p-[clamp(24px,4vw,44px)] shadow-[inset_0_1px_0_rgb(255_255_255_/_58%)] max-[900px]:grid-cols-1">
        <div className="grid gap-7 font-display text-[clamp(16px,1.55vw,20px)] leading-[1.8] tracking-[0.04em] text-[#9a3d18]">
      ...
      <section className="border-t border-line py-[42px]" aria-labelledby="history-main-title">
        <h2 id="history-main-title" className="mb-7 font-display text-[clamp(42px,6vw,76px)] font-normal leading-[1.08] tracking-[0.06em] text-[#8f2e22]">
          {content.title}
        </h2>
        <ol className="relative grid list-none grid-cols-2 gap-x-14 gap-y-5 p-0 before:absolute before:bottom-0 before:left-1/2 before:top-0 before:w-0.5 before:-translate-x-px before:bg-[linear-gradient(var(--color-rose),var(--color-teal))] before:opacity-55 max-[760px]:grid-cols-1 max-[760px]:gap-4 max-[760px]:pl-5 max-[760px]:before:left-[3px]">
          {timeline.events.map((event, index) => (
            <li
              ...
              className={`relative rounded-2xl border border-line/80 bg-[linear-gradient(135deg,rgb(255_253_249_/_96%),rgb(248_240_231_/_82%))] px-6 py-5 shadow-[0_14px_34px_rgb(76_50_28_/_9%)] before:absolute before:top-[30px] before:size-3.5 before:rounded-full before:shadow-[0_0_0_6px_#fff6ee] max-[760px]:col-start-1 max-[760px]:translate-y-0 max-[760px]:before:-left-[21px] ${
                index % 2 === 0
                  ? 'col-start-1 translate-y-2.5 before:-right-[34px] before:bg-rose'
                  : 'col-start-2 translate-y-[54px] before:-left-[34px] before:bg-teal'
              } ${event.continued ? 'bg-[#fffaf4]' : ''}`}
            >
              <time className={`mb-2.5 inline-flex rounded-full bg-[#fbefd9] px-3 py-1.5 text-base font-black leading-none ${event.continued ? 'text-muted' : 'text-[#8f2e22]'}`}>
                {event.date}
              </time>
