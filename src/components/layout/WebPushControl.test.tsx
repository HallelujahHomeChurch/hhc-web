...
describe('WebPushControl', () => {
  ...
  it('binds an existing subscription to the authenticated account without exposing a user id', async () => {
    ...
    vi.stubGlobal('fetch', vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url.endsWith('/session')) {
        return Response.json({authenticated: true, user: {id: 'private-user-id', email: 'user@example.test', display_name: 'User', avatar_url: null, permissions: []}});
      }
  ...
  it('does not repeat successful subscription writes when remounted', async () => {
    ...
    vi.stubGlobal('fetch', vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url.endsWith('/session')) return Response.json({authenticated: true, user: {id: 'user-id', email: 'user@example.test', display_name: 'User', avatar_url: null, permissions: []}});
